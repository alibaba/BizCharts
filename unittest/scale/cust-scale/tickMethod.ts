...
export default cfg => {
  ...
  let { min, max, shrink } = cfg || {};
  min = isNaN(min) ? 0 : min;
  shrink = isNaN(shrink) ? 0 : shrink;

  let count = tickCount && tickCount >= 3 ? (tickCount - 1) : DEFAULT_COUNT;

  // 计算interval， 优先取tickInterval
  const interval = tickInterval || getBestInterval({ tickCount: count, max: shrink, min });
  // 通过interval计算最小tick
  ...
  while (tickLength < count) {
    let t = minTick + tickLength * interval;
    if (t > shrink) {
      t = shrink;
    }
    ticks.push(toFixed(t, fixedLength));
    tickLength++;
  }
  return [...ticks, cfg.max];
};
