...
function FacetSuccessRate({ indexNameColorMap, typeNameMap, onGetG2Instance }) {
  const [data, setData] = useState(myData);
  useEffect(() => {
    () => setData(myData.slice(0,30));
    // setTimeout(() => {
    //   act(() => setData(myData.slice(0,30)));
    // }, 1500)
  }, [])
	return (
		<div>
...
describe('components-Chart', () => {
  ...
  test('数据不变则不更新图表', async () => {
    let chart = null;
    render(<FacetSuccessRate  indexNameColorMap={myIndexNameColorMap} typeNameMap={myTypeNameMap} onGetG2Instance={c => chart = c} />);
