...
###  title 
_<object>_ _<boolean>_ _<null>_
...
通过 `title={true}` 渲染坐标轴标题。通过以下配置对标题进行个性化配置：参考[绘图属性](40)
...
###  line 
_<object>_ _<null>_
* 描述：设置坐标轴线的样式，包括线的颜色、粗细等。如果该属性值为 null 则表示不展示坐标轴线。样式设置细节参考[绘图属性](40)
```js
//可配置样式
{
  stroke: 'dddddd',
...
```
设置alignWithLabel为false且数据类型为 category 时:
![alignWithLabel:false](https://gw.alipayobjects.com/mdn/rms_2274c3/afts/img/A*ZbKGTK5s49oAAAAAAAAAAABkARQnAQ)
