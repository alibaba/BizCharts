...
路径（path)和线(line)都是一维的线，两者间的唯一差别在于线展示的数据是沿 x 轴方向有序的，而路径的数据是无序的。
...
// 在每个图形上显示 y 字段对应的数值
<Path position="x*y" label="y" />
```
- 在每个图形上显示 x 字段对应的数值，同时配置文本颜色为红色
```js
<Path position="x*y" label={["x", { style: { fill: 'red' } }]} />
```
...
```js
<Path positon="x*y" adjust="stack" color="type" />
// 或者
<Path 
  positon="x*y"
  adjust= {[
