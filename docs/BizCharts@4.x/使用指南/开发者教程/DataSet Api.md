...
存储已注册的 Connector（key-value 对），Connector 用法见 [Connector](153)。

<span id="transforms" />
### transforms

存储已注册的 Transform（key-value 对），Transform 用法见 [Transform](154)。
