...
## api文档
详情查看[DataSet Api](152)

DataSet 主要完成了以下功能：
 * 源数据的解析，将 CSV, DSV, GeoJSON 转成标准的 JSON，查看 [Connector](153)。
 * 数据处理，查看 [Transform](154)：
...
> 注意： BizCharts@4.x中data数据不支持dataView数据格式，请使用 data={dv.rows}替代。

### 单独使用 DataView
如果仅仅是对数据进行加工，不需要图表联动

### 状态量
使用 DataSet 的状态量(State) 可以很容易的实现图表的联动，步骤如下：
 ...
 */
$.getJSON('data/population-by-age.csv', data => {
    const dvForAll = ds
