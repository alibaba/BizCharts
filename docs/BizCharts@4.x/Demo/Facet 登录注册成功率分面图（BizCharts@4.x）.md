# Facet 登录注册成功率分面图（BizCharts@4.x）

![预览](http://bizcharts-resource.oss-cn-zhangjiakou.aliyuncs.com/images/6fb49410-59ff-11eb-9b2e-1bd85a98cb49.png)
...
function FacetSuccessRate({ data, indexNameColorMap, typeNameMap }) {
	return (
		<Chart
			autoFit
			data={data}
...
ReactDOM.render(<FacetSuccessRate data={myData} indexNameColorMap={myIndexNameColorMap} typeNameMap={myTypeNameMap} />, mountNode);
