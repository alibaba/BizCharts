...
解构使用
```js
import { StackedRoseChart } from 'bizcharts';
function APP() {
  return <StackedRoseChart {...options} />;
}
```
按需使用
```js
import StackedRoseChart from 'bizcharts/lib/plots/StackedRoseChart'; // es 取 'bizcharts/es/plots/StackedRoseChart'
function APP() {
  return <StackedRoseChart {...options} />;
}
```
