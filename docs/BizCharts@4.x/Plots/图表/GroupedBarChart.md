...
function APP() {
  return <GroupedBarChart {...options} />;
}
```
按需使用
```js
import AreaChart from 'bizcharts/lib/plots/GroupedBarChart'; // es 取 'bizcharts/es/plots/GroupedBarChart'
function APP() {
  return <GroupedBarChart {...options} />;
}
```
